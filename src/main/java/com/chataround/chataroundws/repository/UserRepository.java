...
public interface UserRepository extends JpaRepository<User, Long> {
        @Query(value = "SELECT DISTINCT * FROM user_table WHERE earth_box(ll_to_earth(?1, ?2), ?3) @> ll_to_earth(coordinates_latitude, coordinates_longitude) AND user_table.isonline=TRUE ", nativeQuery = true)
        ...
        @Query(value = "SELECT count(*)>0 FROM user_table WHERE username=?1", nativeQuery =true)
